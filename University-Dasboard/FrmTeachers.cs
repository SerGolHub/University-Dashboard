...
namespace University_Dasboard
{
	public partial class FrmTeachers : Form
	{
		...
		private void btnAdd_Click(object sender, EventArgs e)
		{
			...
			if (string.IsNullOrEmpty(phoneNumber) || !Regex.IsMatch(phoneNumber, "^(\\+7)[0-9]{10}$"))
			{
				logger.Warn("Попытка добавить мобильный телефон не удалась.");
				MessageBox.Show("Номер телефона введен неверно. Он должен начинаться с '+7' и содержать 11 цифр.");
				return;
			}
